...
int main(){
    ...
    for(i=0;i<16;i++){
        for(j=0;j<16;j++){
            cin>>sudoku[i][j];
            int nowchar;
            if(sudoku[i][j]=='-')nowchar=0;
            else nowchar=sudoku[i][j]-'A'+1;
            //cout<<nowchar<<"x\n";
            auto make_row=[&](int x){
                vector<int> row(1024);
                row[16*(i)+j]=1;
                row[256-1+16*(i)+x]=1;
                row[256*2-1+16*(j)+x]=1;
                row[256*3-1+((i)/4*4+(j)/4)*16+x]=1;
                mat.push_back(row);
                row_data.push_back({i,j,x});
            };
            if(nowchar)make_row(nowchar);
            else for(int k=1;k<=16;k++)make_row(k);
        }
    }
    ...
    /*for(Node* it=head->right;it!=head;it=it->right){
        ...
        cout<<it->sz<<" ";if(it->name%16==0)cout<<"\n";
    }*/
    vector<int> solution;
